...
export async function setupDatGui(urlParams) {
  ...
  params.STATE.videoDevices = videoDevices[0];
  
  const videoDevicesController = cameraFolder.add(
    params.STATE, 'videoDevices', Object.values(videoDevices.map(el=>el.deviceId)));
    videoDevicesController.onChange(value => {   
    params.STATE.camera.deviceId={exact:value};
    Camera.setupCamera(params.STATE.camera)
  });  
  cameraFolder.open();
