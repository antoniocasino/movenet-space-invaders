...
export class Camera {
  ...
  drawCircle = (initialPosition)=>{
    this.ctx.beginPath();
    this.ctx.arc(initialPosition.x/2, initialPosition.y/2, 15, 0, 2 * Math.PI, false);
  ...
  async setupCamera(cameraParam) {
    ...
    const videoWidth = camera.video.videoWidth/2;
    const videoHeight = camera.video.videoHeight/2;
    ...
    camera.video.addEventListener('play', () => {
      camera.drawCtx();
      camera.timerCallback();
    }, false);
  ...
  drawCtx() {
    this.ctx.drawImage(
        this.video, 0, 0, this.video.videoWidth/2, this.video.videoHeight/2);
  }
